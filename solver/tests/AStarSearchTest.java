...
class AStarSearchTest {
    ...
    void ThreeByThreeAStarSearch() {
        ...
        ArrayList<Integer> p1 = search.shortestPath(0, 8, g, new ArrayList<>(), true);
        ArrayList<Integer> p2 = search.shortestPath(0, 8, g, new ArrayList<>(), false);

        System.out.println("3 to 0 (prioritize row):\t\t\t" + p1);
        System.out.println("3 to 0 (prioritize col):\t\t\t" + p2);

        assertAll(
                () -> assertArrayEquals(new ArrayList<>(Arrays.asList(8, 5, 2, 1, 0)).toArray(), p1.toArray()),
                () -> assertArrayEquals(new ArrayList<>(Arrays.asList(8 ,7, 6, 3, 0)).toArray(), p2.toArray())
        );
    } // end of TwoByTwoAStarSearch()
    ...
    void ThreeByThreeWithBlocksAStarSearch() {
        ...
        ArrayList<Integer> p4 = search.shortestPath(0, 8, g, new ArrayList<>(List.of(2)), false);
        ArrayList<Integer> p5 = search.shortestPath(0, 8, g, new ArrayList<>(List.of(4, 5)), true);

        System.out.println("3 to 0 without [1] (prioritize row):\t" + p3);
        System.out.println("3 to 0 without [2] (prioritize col):\t" + p4);
        System.out.println("3 to 0 without [4,5] (prioritize col):\t" + p5);

        assertAll(
                () -> assertArrayEquals(new ArrayList<>(Arrays.asList(8, 5, 4, 3, 0)).toArray(), p3.toArray()),
                () -> assertArrayEquals(new ArrayList<>(Arrays.asList(8, 7, 6, 3, 0)).toArray(), p4.toArray()),
